...
let demosView (model:Model) dispatch =
    ...
    p.onSortedChange <- ReactTable.SortedChangeFunction(fun newSort col additive ->
        JS.console.log("DemosView.onSortedChange called..")
        let rule = newSort.[0]
    ...
    p.onPageSizeChange <- (fun newPageSize newPage ->
        JS.console.log("DemosView.onPageSizeChange called..")
        let page = int newPage
        let newPageSize = int newPageSize
        let newStart = newPageSize * page
        startLoadingData newStart newPageSize model.SortingRule
    )

    p.onPageChange <- (fun newPage ->
        JS.console.log("DemosView.onPageChange called..")
        let page = int newPage
        let newStart = model.CurrentPageSize * page
        let newPageSize = model.CurrentPageSize
        startLoadingData newStart newPageSize model.SortingRule
    )
    let toogle key =
        let current =
...
let view (model : Model) (dispatch : Msg -> unit) =
    JS.console.log("DemosView.view called..")
